package com.xadmin.transportcompany.bean;
...
public class MyListener implements ServletContextListener{
	...
	public void contextInitialized(ServletContextEvent arg0) {
		...
		try{
				...
				PreparedStatement ps2= con.prepareStatement("CREATE SEQUENCE SEQUENCE_1 MINVALUE 1 MAXVALUE 999999999 INCREMENT BY 1 START WITH 1 NOCACHE NOORDER NOCYCLE");
				ps2.executeUpdate();
				...
				stmt.executeUpdate("CREATE TRIGGER  BI_PAYREGISTER before insert on PAYREGISTER for each row begin select SEQUENCE_1.nextval into :NEW.ID from dual;end");
				stmt.executeUpdate("CREATE TRIGGER  BI_TINSTALL before insert on TINSTALL for each row begin select SEQUENCE_1.nextval into :NEW.ID from dual;end");
